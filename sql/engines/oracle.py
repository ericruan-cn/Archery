...
class OracleEngine(EngineBase):
    ...
    def _get_all_schemas(self):
        ...
        :return:
        """
        result = self.query(sql="SELECT username FROM all_users order by created")
        sysschema = (
            ...
            'SYSMAN', 'SYSTEM', 'WMSYS', 'XDB', 'XS$NULL', 'WMSTOLD',
            'DSG', 'DIP', 'SPOTLIGHT', 'ZABBIX', 'DBMONITOR', 'LDCFGTAB', 'WULIU', 'LBACSYS',
            'NEWCS', 'HQSJJK', 'HQLNDYWP', 'HQLNDWMS', 'HQLNDYWD', 'HQLNDYWA', 'HQLNDYWB',
            'HQLNDYWC', 'WMSTOLD ', 'HQCCTOLD', 'HQLNDAUD', 'HQSCMYW', 'HQLNDYG', 'QUF_MALL')
    ...
    def execute_check(self, db_name=None, sql=''):
        ...
        # 把所有SQL转换成SqlItem List。 如有多行执行块，则约定以delimiter $$作为开始分割符, 以$$结束
        sqlitemList = get_full_sqlitem_list(sql, db_name)

        for sqlitem in sqlitemList:
            # 禁用语句
            if re.match(r"^select", sqlitem.statement.lower()):
    ...
    def execute_workflow(self, workflow, close_conn=True):
        """执行上线单，返回Review set
           原来的逻辑是根据 sql_content简答来分割SQL，进而再执行这些SQL
           新的逻辑变更为根据审核结果中记录的sql来执行，如果是PLSQL存储过程等的定义操作，
           还需要确认检查新建对象编译是否有错!
        """
